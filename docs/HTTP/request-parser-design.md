...
flowchart TD
    ...
    K --> B
    C -->|"FAILED"| D["Connection builds error HttpResponse from getErrorStatus()"]
    C -->|"DONE"| E["HttpParser::request()"]
...
- `_path`에는 query string이 들어가지 않는다.
...
class HttpParser {
public:
    enum State { READING_HEAD, READING_BODY, DONE, FAILED };
...
| `getErrorStatus()` | `FAILED`일 때 HTTP status code를 반환한다. |
| `request()` | `DONE`일 때 완성된 `HttpRequest`를 반환한다. |
...
| `DONE` | `HttpRequest`가 완성됐다. |
| `FAILED` | 잘못된 요청이며 `getErrorStatus()`로 응답 status를 확인한다. |
