...
`HttpSyntax`는 Owner A 내부의 HTTP request 문법 helper다. header field 문법, Host hostname 검증, request-target 검증, percent-decoding, dot-segment 정규화, chunk size hex 파싱처럼 Parser/Request 모델 구현에 필요한 HTTP syntax 처리를 담당하며, 다른 owner가 직접 의존해야 하는 공용 프로젝트 API로 보지 않는다.
...
Parser 내부와 URI syntax helper는 raw integer 대신 `HttpStatus`를 사용한다. `HttpParser::errorStatus()`는 기존 C/C++ 통합 편의를 위해 `int`를 반환하되, 내부 저장값은 `HttpStatus`다.
...
| `getHost()` | `Host` 헤더에서 분리하고 lowercase로 정규화한 hostname을 반환한다. 예: `"localhost"`. |
...
| `setRequestLine()` | method, uri, version을 저장하고 `splitUri()`를 호출한다. URI 검증 또는 정규화에 실패하면 해당 `HttpStatus`를 반환한다. |
| `addHeader()` | header name을 lowercase로 정규화해서 저장한다. |
| `setHost()` | Parser가 `Host` 헤더에서 분리하고 lowercase로 정규화한 hostname을 저장한다. |
...
- `_host`에는 lowercase로 정규화한 `Host` 헤더의 hostname만 저장한다. `host:port`의 `:port`는 들어가지 않는다.
...
- path 안의 raw backslash와 percent-encoded slash/backslash 차단
- Host hostname 문법 검증 및 lowercase 정규화
- 중복/충돌 헤더 정책
...
| 함수 | 담당 |
| --- | --- |
| `matchServer()` | 같은 port의 server 중 host가 `serverNames`와 case-insensitive로 일치하는 server를 반환한다. |
| `matchLocation()` | `path`와 가장 길게 segment-boundary prefix match되는 location을 반환한다. |

### `matchServer` 규칙

1. `port`가 일치하는 server만 후보로 본다.
2. `host`가 `serverNames` 중 하나와 case-insensitive로 같으면 해당 server를 반환한다.
3. 같은 port에 host 일치가 없으면 같은 port의 첫 번째 server를 default server로 반환한다.
4. 같은 port의 server가 없으면 `NULL`을 반환한다.

`Host` 헤더에 port가 붙어 있는 경우, 예를 들어 `localhost:8080`, Parser가 `localhost`와 `8080`으로 split해서 `HttpRequest`에 저장한다. Parser는 hostname을 lowercase로 정규화하고, Router는 `request.getHost()`의 hostname만 server name matching에 사용한다. `serverNames`와의 비교는 case-insensitive이며, routing 대상 port는 listening socket에서 결정된 port를 사용한다.
...
```cpp
_state = FAILED;
_errorStatus = HTTP_STATUS_BAD_REQUEST;
```

Owner A가 직접 발생시킬 수 있는 status 기준:

| 단계 | 상황 | status |
| --- | --- | --- |
| Parser 상태 | parser 성공, 아직 response status 없음 | `HTTP_STATUS_NONE` |
| request-line | 빈 request-line | `HTTP_STATUS_BAD_REQUEST` |
| request-line | SP 구분자가 없거나, 필드가 3개가 아니거나, tab으로 구분됨 | `HTTP_STATUS_BAD_REQUEST` |
| method | method가 HTTP token 문법이 아님 | `HTTP_STATUS_BAD_REQUEST` |
| method | token 문법은 맞지만 Owner A가 지원하지 않는 method (`GET`, `POST`, `DELETE` 외) | `HTTP_STATUS_NOT_IMPLEMENTED` |
| request-target | origin-form이 아님 (`/`로 시작하지 않음) | `HTTP_STATUS_BAD_REQUEST` |
| request-target | fragment(`#`) 포함 | `HTTP_STATUS_BAD_REQUEST` |
| request-target | raw target 또는 query에 공백/control char 포함 | `HTTP_STATUS_BAD_REQUEST` |
| request-target | percent triplet 문법 오류 (`%zz`, `%2` 등) | `HTTP_STATUS_BAD_REQUEST` |
| request-target | raw backslash 또는 path 안의 percent-encoded slash/backslash (`%2F`, `%5C`) | `HTTP_STATUS_BAD_REQUEST` |
| request-target | percent-decoding 후 path에 control char 포함 | `HTTP_STATUS_BAD_REQUEST` |
| request-target | path 정규화 중 root 밖으로 올라가는 dot-segment (`/../x`, `/%2e%2e/x`) | `HTTP_STATUS_FORBIDDEN` |
| request-target | request-target 길이 제한 초과 | `HTTP_STATUS_URI_TOO_LONG` |
| version | `HTTP/1.1` | success |
| version | HTTP-version 문법은 맞지만 지원하지 않음 (`HTTP/1.0`, `HTTP/2.0` 등) | `HTTP_STATUS_HTTP_VERSION_NOT_SUPPORTED` |
| version | HTTP-version 문법 자체가 틀림 (`HTP/1.1`, `HTTP/1`, `HTTP/1.x` 등) | `HTTP_STATUS_BAD_REQUEST` |
| header section | `\r\n\r\n` 전 header section 제한 초과 | `HTTP_STATUS_REQUEST_HEADER_FIELDS_TOO_LARGE` |
| header field | field line 길이 제한 초과 | `HTTP_STATUS_REQUEST_HEADER_FIELDS_TOO_LARGE` |
| header field | header 개수 제한 초과 | `HTTP_STATUS_REQUEST_HEADER_FIELDS_TOO_LARGE` |
| header field | colon 없음, 빈 field-name, colon 앞 whitespace | `HTTP_STATUS_BAD_REQUEST` |
| header field | field-name이 token 문법이 아님 | `HTTP_STATUS_BAD_REQUEST` |
| header field | field-value에 허용되지 않는 control char 포함 | `HTTP_STATUS_BAD_REQUEST` |
| Host | `Host` 없음, 빈 값, whitespace 포함 | `HTTP_STATUS_BAD_REQUEST` |
| Host | `host:port`에서 host 또는 port가 비어 있음 | `HTTP_STATUS_BAD_REQUEST` |
| Host | hostname에 ASCII `A-Z`, `a-z`, `0-9`, `.`, `-`, `_` 외 문자가 있거나 alnum 문자가 하나도 없음 | `HTTP_STATUS_BAD_REQUEST` |
| Host | port가 숫자가 아니거나 65535 초과 | `HTTP_STATUS_BAD_REQUEST` |
| Host | IPv6 literal처럼 colon이 2개 이상인 현재 미지원 형식 | `HTTP_STATUS_BAD_REQUEST` |
| duplicate header | `Host` 중복 | `HTTP_STATUS_BAD_REQUEST` |
| duplicate header | `Transfer-Encoding` 중복 | `HTTP_STATUS_BAD_REQUEST` |
| duplicate header | `Content-Length` 중복 값이 서로 다름 | `HTTP_STATUS_BAD_REQUEST` |
| Content-Length | 빈 값, 숫자 외 문자 포함 | `HTTP_STATUS_BAD_REQUEST` |
| Content-Length | 정수 overflow 또는 body limit 초과 | `HTTP_STATUS_CONTENT_TOO_LARGE` |
| Transfer-Encoding | `Content-Length`와 동시에 존재 | `HTTP_STATUS_BAD_REQUEST` |
| Transfer-Encoding | 값이 비어 있음 | `HTTP_STATUS_BAD_REQUEST` |
| Transfer-Encoding | 문법상 field는 맞지만 Owner A가 지원하지 않는 transfer coding (`chunked` 외) | `HTTP_STATUS_NOT_IMPLEMENTED` |
| fixed body | `Content-Length` body가 설정 limit보다 큼 | `HTTP_STATUS_CONTENT_TOO_LARGE` |
| chunked body | chunk-size line 제한 초과 | `HTTP_STATUS_BAD_REQUEST` |
| chunked body | chunk-size가 비었거나 hex가 아님 | `HTTP_STATUS_BAD_REQUEST` |
| chunked body | chunk-size 정수 overflow 또는 decoded body limit 초과 | `HTTP_STATUS_CONTENT_TOO_LARGE` |
| chunked body | chunk data 뒤 CRLF 누락 | `HTTP_STATUS_BAD_REQUEST` |
| chunked trailer | trailer field line 제한 초과 | `HTTP_STATUS_REQUEST_HEADER_FIELDS_TOO_LARGE` |
| chunked trailer | trailer field 문법 오류 | `HTTP_STATUS_BAD_REQUEST` |

Owner A가 직접 발생시키지 않는 status:

| status | 이유 |
| --- | --- |
| `405 Method Not Allowed` | route별 method 허용 여부는 `LocationConfig`와 handler/response 단계에서 판단한다. |
| `408 Request Timeout` | timeout은 `Connection`/event loop가 시간 기준으로 판단한다. |
| `411 Length Required` | POST/upload 정책상 body length를 요구할지는 handler 정책이다. Parser는 body가 없으면 length 0으로 본다. |
| `404 Not Found` | routing 이후 file/resource 존재 여부는 handler가 판단한다. |
| `500`, `502`, `504` | 내부 오류와 CGI 오류/timeout은 response/CGI integration 단계 책임이다. |
...
| request-target 지원 형식 | origin-form만 허용한다. path는 percent-decoding 후 dot-segment 정규화하고, `/..`로 root 밖을 가리키면 `403`이다. |
| encoded path separator | path 안의 `%2F`, `%5C`는 decoded path separator로 취급하지 않고 `400`으로 거부한다. raw backslash도 `400`으로 거부한다. |
| Host hostname 검증 | hostname은 ASCII `A-Z`, `a-z`, `0-9`, `.`, `-`, `_`만 허용하고 alnum 문자가 하나 이상 있어야 한다. |
| Host/server_name 비교 | Host hostname은 lowercase로 저장하고, Router는 `serverNames`와 case-insensitive로 비교한다. |
